...
namespace PerformanceTestFilters
{
   /// <summary>
   ...
   public class GaussianBlurEffect : IEffect
   {
      ...
      public WriteableBitmap Process(WriteableBitmap input)
      {
         ...
         input.CopyPixels(pixelData, widthInBytes, 0);
         for (int i = 0; i < pixelData.Length; ++i)
         {
             pixelData[i] ^= 0x00ffffff;
         }
         input.WritePixels(new Int32Rect(0, 0, w, h), pixelData, widthInBytes, 0);
         pixelData = Process(pixelData, width, height);
         byte[] result = new byte[pixelData.Length * sizeof(int)];
         Buffer.BlockCopy(pixelData, 0, result, 0, result.Length);
         return input.FromByteArray(result);
      }
